...
parser.add_argument('-i', '--index', type=int, help="index of the sentence we want to deconcatenate (Starts at 0)")
parser.add_argument('-s', '--save', action='store_true', default=False, help="Optional parameter to save previous sentences (with respect to --index) in a separated file")
...
for i in range(len(fl)):
    ...
    if i == 0 and save: # If it is the first line do nothing. I think I should remove this line, why is this shit here ? 
        print()
    elif i == len(fl) - 1 and save: #If it is the last line we need to save both sentences
        fp.write(split_s[idx-1]+'\n')
        fp.write(split_s[idx]+'\n')
